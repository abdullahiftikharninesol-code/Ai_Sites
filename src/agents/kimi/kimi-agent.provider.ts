...
import type { KimiAgentConfig } from "./kimi-agent.config.js";
export class KimiAgentProvider implements AgentProvider {
  readonly id = "kimi";
  readonly #client: ChatCompletionsClient;
  constructor(
    readonly config: KimiAgentConfig,
    readonly dependencies: CompatibleProviderDependencies = {},
  ) {
    this.#client =
      dependencies.client ?? createCompatibleClient(config, "https://api.moonshot.ai/v1");
  }
  getCapabilities() {
    return {
